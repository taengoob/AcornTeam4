...
public class OrderRequestController {
    ...
    public String AddOrderRequest(Model model) {
        Object obj = model.getAttribute("orderRequestList");

        if (obj instanceof List<?>) {
            List<?> orderRequestList = (List<?>) obj;
            for (Object orderRequest : orderRequestList) {
                if (orderRequest instanceof OrderRequestDTO)
                    service.insertOrderRequest((OrderRequestDTO) orderRequest);
            }
        }
